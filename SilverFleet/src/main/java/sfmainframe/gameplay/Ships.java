...
public final class Ships {
	 ...
	 * [8.3].
	 */
	private static final List<Double> WIND_FACTOR = Collections
			.unmodifiableList(Arrays.asList(new Double[] { 3.5, 5.0, 7.0 }));

	private static Game game;

	static void coupleShips(Ship shipOne, Ship shipTwo, CoupleReason reason) {
		MainBoard.addMessage("Ship #" + shipOne.getID() + " and ship #"
				+ shipTwo.getID() + " coupled (" + reason.toString() + ").\n");
	...
	public static void throwTow(Ship ship) {
		// W danej chwili możliwa jest tylko jedna opcja (holownik albo
		// holowany).
	...
	static void sinkShip(Ship ship, DestroyShipMode mode) {
		...
		// par. 5.3.5
		for (MarinesCompartment location : MarinesCompartment
				.getShipCompartments()) {
		...
		if (ship.getInternedBy() != Player.NONE)
			game.getPlayer(ship.getInternedBy()).removeInternedShip(
					ship.getOwner(), ship);
	...
	public static void blowUpShip(Ship ship) {
		...
		for (Ship s : ship.getShipsCoupled().keySet()) {
			...
			RotateDirection rotations[] = { RotateDirection.NE,
					RotateDirection.SE, RotateDirection.SW, RotateDirection.NW };
			...
			if (dA == -1 && dB == -1
					&& ship.getRotation() != RotateDirection.NE
					&& ship.getRotation() != RotateDirection.SW)
				angleModifier = 4;
			...
			if (angleModifier != null) {
				for (int dR = 0; dR < 6; dR++) {
					if (rotations[dR] == RotateDirection.rotate(
							s.getRotation(), angleModifier)) {
			...
			for (Player plr : Player.getValues()) {
				killMarines(s, plr, MarinesCompartment.DECK,
						rollDice(s, shipOwner), KillingMode.WITH_COMMANDER);
			}
	...
	public static boolean destroyMast(Ship ship, int points) {
		...
		MainBoard.addMessage(String.format(
				"Ship #%d: speed fell by %d knots\n", ship.getID(),
	...
	public static boolean destroyHelm(Ship ship, int points) {
		boolean damage = false;
		MainBoard.addMessage("Ship #" + ship.getID()
				+ ": manoeuvrability fell by "
	...
	public static boolean destroyHeaviestCannon(Ship ship,
			GunCompartment location) {
	...
	public static boolean damageHull(Ship ship, int points) {
		...
		MainBoard.addMessage("Ship #" + ship.getID() + ": hull damage ("
				+ points + " pts)\n");
	...
	public static boolean checkIfStillTowable(Ship tug, Ship towed) {
		...
		if (!checkIfPlayerControlsLocation(tug, game.getCurrentPlayer(),
				MarinesCompartment.DECK, true)
				|| !checkIfPlayerControlsLocation(towed,
						game.getCurrentPlayer(), MarinesCompartment.DECK, true))
	...
	public static boolean checkIfTowable(Ship tug, Ship towed) {
		...
		if (!checkIfStillTowable(tug, towed))
			return false;
		if (tug.getTowedBy() != null || tug.getTowOther() != null
				|| towed.getTowedBy() != null || towed.getTowOther() != null)
			return false; // par. 16.3
		if (Math.abs(tugPos.getA() - towedPos.getA()) > 1
				|| Math.abs(tugPos.getB() - towedPos.getB()) > 1
				|| getAlliedMarinesNumber(tug, game.getCurrentPlayer(),
						MarinesCompartment.DECK) < 2
				|| getAlliedMarinesNumber(towed, game.getCurrentPlayer(),
						MarinesCompartment.DECK) < 2)
	...
	public static boolean checkIfPlayerControlsLocation(Ship ship,
			Player player, MarinesCompartment location, boolean withAllies) {
		/*
		...
		if (withAllies && getAlliedMarinesNumber(ship, player, location) == 0)
			return false;
				...
				&& ship.getMarinesNumber(player, location, Commons.BOTH) == 0)
			return false;
		else
			return true;
	}

	public static int getAlliedMarinesNumber(Ship ship, Player player,
			MarinesCompartment location) {
		/*
	...
	public static boolean destroyCannonIS(Ship ship, GunCompartment location,
			Gun type, MsgMode mode) {
		ship.destroyCannon(location, type, Commons.BOTH);

		if (mode == MsgMode.ON)
			MainBoard.addMessage("Ship #" + ship + ": " + type.toString()
					+ " cannon from " + location.toString()
					+ " compartment lost.\n");
	...
	public static List<Object> calculateShipOwner(Ship ship) {
		...
		if (ship.getOwner() != Player.NONE
				&& ship.getPlayerMarinesOnShip(ship.getOwner(), false) > 0) {
		...
		if (playersOnBoard == 0) {
			...
			MainBoard
					.addMessage("Ship #" + ship.getID() + ": is now a wreck\n");
		...
		for (Player plr : Player.values()) {
			if (ship.getPlayerMarinesOnShip(plr, false) > ship
					.getPlayerMarinesOnShip(winner, false)
					|| ship.getPlayerMarinesOnShip(plr, false) == ship
							.getPlayerMarinesOnShip(winner, false)
					&& Dice.roll() > 3)
		...
		if (playersOnBoard == 1
				&& ship.getPlayerMarinesOnShip(ship.getOwner(), true) == 1
				|| winner == ship.getOwner()) {
			winnerId = PREVIOUS_OWNER;
			return result;
		}
		...
		for (i = first + 1; i < Commons.PLAYERS_MAX; i++) {
			if (!game.getPlayer(Player.valueOf(first))
					.isAlly(Player.valueOf(i)))
				break;
		}
				...
				for (Player plr : Player.values()) {
					...
					if (game.getPlayer(winner).isAlly(plr)) {
						...
						if (ship.getCommanderState(plr, loc) != CommanderState.NOT_THERE) {
							ship.setCommander(plr, loc,
									CommanderState.NOT_THERE);
							ship.setCommander(plr, MarinesCompartment.INMOVE,
									CommanderState.IMPRISONED);
						}
					}
				}
			}
		...
		MainBoard.addMessage("Ship #" + ship.getID() + ": new owner is "
				+ winner.toString() + "\n");
	...
	public static int killMarines(Ship ship, Player player,
			MarinesCompartment location, int number, KillingMode mode) {
		/*
		...
		if (player != Player.NONE) {
			...
			MainBoard.addMessage("Ship #" + ship.getID() + ": " + killed
					+ " marines ");
			...
			for (Player plr : Player.values()) {
				...
				if (ship.getMarinesNumber(plr, location, Commons.BOTH) > 0
					...
					totalNumber += ship.getMarinesNumber(plr, location,
							Commons.BOTH);
			...
			for (int i = 0; i < plrs.size(); i++) {
				int thisNumber = ship.getMarinesNumber(plrs.get(i), location,
						Commons.BOTH);
				if (ship.getCommanderState(plrs.get(i), location) != CommanderState.NOT_THERE)
					thisNumber += 1;

				Object[] ret = ship.killMarines(player, location,
						(int) (Math.ceil(thisNumber / totalNumber)), mode);
				killed = (Integer) ret[0];
				if (ret[1] == KillingMode.WITH_COMMANDER) {
					if (mode == KillingMode.WITH_COMMANDER)
	...
	private static boolean runDown(Ship aggressor, Ship victim) {
		 ...
		 */
		MainBoard.addMessage("Ship #" + aggressor.getID() + ": rammed ship #"
				+ victim.getID() + "\n");
	...
	public static void handle(Ship source, Ship target, Player player,
			CargoDestination from, CargoDestination to, CargoType cargoType,
			GunCompartment sourceCompartment, GunCompartment targetCompartment,
			int quantity) {
		...
		if (cargoType == CargoType.SILVER) {
			...
			source.moveMarines(player, MarinesCompartment.INMOVE,
					MarinesCompartment.INMOVE, quantity);
			return;
		}
		...
		if (cargoType == CargoType.CANNONS_LIGHT) {
			source.moveMarines(player, MarinesCompartment.INMOVE,
					MarinesCompartment.INMOVE, quantity * 10);
			if (to == CargoDestination.CARGO)
				target.loadCargo(CargoType.CANNONS_LIGHT, quantity);
			else
				target.modifyCannonsNumber(targetCompartment, Gun.LIGHT,
						quantity);
		}
		if (cargoType == CargoType.CANNONS_MEDIUM) {
			source.moveMarines(player, MarinesCompartment.INMOVE,
					MarinesCompartment.INMOVE, quantity * 20);
			if (to == CargoDestination.CARGO)
				target.loadCargo(CargoType.CANNONS_MEDIUM, quantity);
			else
				target.modifyCannonsNumber(targetCompartment, Gun.MEDIUM,
						quantity);
		}
	}
	...
	public static boolean checkIfSetExplosivesPossible(Ship ship, Player player) {
		...
		if (getCompartmentAllies(ship, player, MarinesCompartment.DECK) != CompartmentAllies.NONE
				...
				|| getCompartmentAllies(ship, player,
						MarinesCompartment.BATTERIES) != CompartmentAllies.NONE)
			return false;
		// --
	...
	public static void tow(Ship tug, Ship towed) {
		MainBoard.addMessage("Ship #" + tug.getID() + ": now towing ship #"
				+ towed.getID() + "\n");
	...
	public static void surrenderMarines(Ship ship, MarinesCompartment location,
			int number) {
		// par. 12.5
		killMarines(ship, game.getCurrentPlayer(), location, number,
				KillingMode.WITHOUT_COMMANDER);
		calculateShipOwner(ship);
		// --
	}

	public static void moveMarinesShip(Ship sourceShip, Ship targetShip,
		...
		if (targetShip.getOwner() != Player.NONE
				&& !game.getPlayer(game.getCurrentPlayer()).isAlly(
						targetShip.getOwner())) {
		...
		sourceShip.moveMarines(game.getCurrentPlayer(),
				MarinesCompartment.DECK, MarinesCompartment.SHIP_X, amount);
		targetShip.moveMarines(game.getCurrentPlayer(),
				MarinesCompartment.SHIP_X, MarinesCompartment.DECK, amount);
		MainBoard.addMessage("Ship #" + targetShip.getID() + ": " + amount
				+ " marines moved from ship #" + sourceShip.getID() + "\n");
	...
	public static void moveCommanderShip(Ship sourceShip, Ship targetShip) {
		...
		sourceShip.moveCommander(game.getCurrentPlayer(),
				MarinesCompartment.DECK, MarinesCompartment.SHIP_X);
		targetShip.moveCommander(game.getCurrentPlayer(),
				MarinesCompartment.SHIP_X, MarinesCompartment.DECK);
		MainBoard.addMessage("Ship #" + targetShip.getID()
				+ ": commander moved from ship #" + sourceShip.getID() + "\n");
	...
	public static void closeCombat(Ship ship, Player player, String enemiesStr,
			MarinesCompartment location) {
		int marinesLoss = rollDice(ship, player) * rollDice(ship, player);

		// par. 12.2.2.3
		if (game.getCurrentPlayer() == ship.getOwner()
				&& ship.isBoardingFirstTurn() == BoardingFirstTurn.YES)
			marinesLoss *= 2;
		// --
		...
		for (int i = 0; i < enemiesStrs.length; i++) {
			for (Player plr : Player.values()) {
				if (plr.toString().equals(enemiesStrs[i]))
					killMarines(ship, plr, location, marinesLoss
							/ enemiesStrs.length, KillingMode.WITHOUT_COMMANDER);
				break;
			}
		}
		// --
	...
	public static void makeShallowEscapeAttempt(Ship ship, ShallowAttempt type,
			Ship towed) {
		...
		if (ship.isParameter(Parameter.IS_IMMOBILIZED)) {
			...
			if (type == ShallowAttempt.PULL_ANCHOR
					|| type == ShallowAttempt.TOW_BY_ONE)
				return;

			if (type == ShallowAttempt.DROP_SILVER
					&& rollDice(ship, currentPlayer) > 2)
				attemptSuccessful = true; // par. 17.8.2
			if (type == ShallowAttempt.DROP_CANNONS
					&& rollDice(ship, currentPlayer) > 1)
				attemptSuccessful = true; // par. 17.9.2
			if (type == ShallowAttempt.TOW_BY_BOATS
					&& rollDice(ship, currentPlayer)
							+ rollDice(ship, currentPlayer) > 10)
				attemptSuccessful = true; // 17.12.1

			if (attemptSuccessful) {
				MainBoard.addMessage("Ship #" + ship.getID()
						+ ": escaped from treachous waters\n");
				ship.escapeFromShallow();
				return;
			} else
				MainBoard.addMessage("Ship #" + ship.getID()
						+ ": failed to escape from treachous waters\n");
		} else {
			towed.makeEscapeAttempt(type);
		}
	}

	public static boolean checkIfEscapeAttemptPossible(Ship ship,
			ShallowAttempt type, Ship towed) {
		if (!ship.isParameter(Parameter.IS_IMMOBILIZED)) {
			...
			if (type == ShallowAttempt.TOW_BY_ONE) {
				if (!towed.isParameter(Parameter.IS_IMMOBILIZED)
						...
						|| !(ship.getMovesQueueCode() == MovesQueueCode.NEW || ship
	...
	public static int checkMaxQuantityToHandle(Ship source, Ship target,
			CargoDestination from, CargoDestination to, CargoType cargoType,
			GunCompartment sourceCompartment, GunCompartment targetCompartment) {
		int max = 0;
		Gun gunType = Gun.NONE;
		...
		if (cargoType == CargoType.SILVER) {
			...
			max = Math.min(max, source.getMarinesNumber(currentPlayer,
					MarinesCompartment.INMOVE, Commons.READY));
		...
		if (from == CargoDestination.CARGO)
			...
			max = source.getCannonsNumber(sourceCompartment, gunType,
					Commons.BOTH);

		if (cargoType == CargoType.CANNONS_LIGHT) {
			max = Math.min(max, source.getMarinesNumber(currentPlayer,
					MarinesCompartment.INMOVE, Commons.READY) / 10);
			if (to == CargoDestination.CARGO)
				...
				// par. 14.3
				int can = target.getShipClass().getCannonMax()[targetCompartment
						.ordinal()][Gun.LIGHT.ordinal()]
						+ target.getShipClass().getCannonMax()[targetCompartment
								.ordinal()][Gun.MEDIUM.ordinal()]
						+ target.getShipClass().getCannonMax()[targetCompartment
								.ordinal()][Gun.HEAVY.ordinal()]
						- target.getCannonsNumber(targetCompartment, Gun.LIGHT,
								Commons.BOTH)
						- target.getCannonsNumber(targetCompartment,
								Gun.MEDIUM, Commons.BOTH)
						- target.getCannonsNumber(targetCompartment, Gun.HEAVY,
								Commons.BOTH);
		...
		if (cargoType == CargoType.CANNONS_MEDIUM) {
			max = Math.min(max, source.getMarinesNumber(currentPlayer,
					MarinesCompartment.INMOVE, Commons.READY) / 20);
			if (to == CargoDestination.CARGO)
	...
	public static boolean checkIfSabotageSuccessful(Ship ship, Player player,
			MarinesCompartment location) {
		...
		for (Player plr : Player.values()) {
			if (plr == Player.NONE)
				continue;
			if (!game.getPlayer(player).isAlly(plr))
				enemyMarinesNumber += ship.getMarinesNumber(plr, location,
						Commons.BOTH);
		}

		// par. 12.2.3.1
		if (enemyMarinesNumber == 0
				&& rollDice(ship, player) + rollDice(ship, player) < ship
						.getMarinesNumber(player, location, Commons.BOTH))
			return true;
		// --
		// par. 12.2.3.2
		if (enemyMarinesNumber > 0
				&& rollDice(ship, player) + rollDice(ship, player)
						+ rollDice(ship, player) < ship.getMarinesNumber(
	...
	private static CompartmentAllies getCompartmentAllies(Ship ship,
			Player player, MarinesCompartment location) {
		/*
	...
	public static void moveCommander(Ship ship, MarinesCompartment source,
			MarinesCompartment destination) {
		ship.moveCommander(game.getCurrentPlayer(), source, destination);
		MainBoard.addMessage("Ship #" + ship.getID()
				+ ": commander moved from " + source.toString() + " to "
	...
	static void storm(Ship ship) {
		...
		int eventType = rollDice(ship, ship.getOwner())
				+ rollDice(ship, ship.getOwner())
				+ rollDice(ship, ship.getOwner());
		Player shipOwner = ship.getOwner();

		MainBoard.addMessage("Ship #" + ship + ": storm event #" + eventType
				+ "happen\n");
		...
		if (eventType == 5) {
			if (destroyHeaviestCannon(ship, GunCompartment.SIDE_L)) {
				killMarines(ship, Player.NONE, MarinesCompartment.BATTERIES, 4,
						KillingMode.WITHOUT_COMMANDER);
				calculateShipOwner(ship);
			}
			return;
		...
		if (eventType == 6) {
			while (destroyCannonIS(ship, GunCompartment.BOW, Gun.LIGHT,
					MsgMode.ON)) {
				killMarines(ship, Player.NONE, MarinesCompartment.BATTERIES,
						Gun.LIGHT.getCrewSize(), KillingMode.WITHOUT_COMMANDER);
				calculateShipOwner(ship);
			}

			while (destroyCannonIS(ship, GunCompartment.BOW, Gun.MEDIUM,
					MsgMode.ON)) {
				killMarines(ship, Player.NONE, MarinesCompartment.BATTERIES,
						Gun.MEDIUM.getCrewSize(), KillingMode.WITHOUT_COMMANDER);
				calculateShipOwner(ship);
			}
			return;
		}
		...
		if (eventType == 8) {
			int cannons = ship.getCannonsNumber(GunCompartment.STERN,
					Gun.LIGHT, Commons.BOTH);
			killMarines(ship, Player.NONE, MarinesCompartment.BATTERIES,
					Gun.LIGHT.getCrewSize() * Math.min(cannons, 1),
					KillingMode.WITHOUT_COMMANDER);
			calculateShipOwner(ship);
		...
		if (eventType == 11) {
			killMarines(ship, Player.NONE, MarinesCompartment.DECK, 1,
		...
		if (eventType == 16) {
			killMarines(ship, Player.NONE, MarinesCompartment.DECK,
					rollDice(ship, shipOwner), KillingMode.WITH_COMMANDER);
			calculateShipOwner(ship);
			return;
		}

		if (eventType == 17) {
			killMarines(ship, Player.NONE, MarinesCompartment.DECK, 10,
					KillingMode.WITH_COMMANDER);
			calculateShipOwner(ship);
			return;
		}
	...
	public static boolean checkIfShipBoarded(Ship ship, Player player) {
		for (MarinesCompartment comp : MarinesCompartment.values()) {
			if (comp == MarinesCompartment.SHIP_X
					|| comp == MarinesCompartment.NONE)
				continue;

			if ((ship.getMarinesNumber(player, comp, Commons.BOTH) > 0
					|| ship.getCommanderState(player, comp) == CommanderState.READY || ship
					.getCommanderState(player, comp) == CommanderState.USED)
					&& DataExtractors.getEnemyGroups(ship,
							game.getPlayer(player), comp).length() > 0)
				return true;
		}
	...
	public static void moveShip(Ship ship, int distance) {
		Player currentPlayer = game.getCurrentPlayer();
		...
		if (!Board.isOnMap(position.sum(shift))) {
			...
			if (shipTowedBy != null) {
				...
				if (shipTowedBy.getOwner() != ship.getOwner())
					...
					// holownikiem.
					MainBoard.addMessage("Ship #" + shipTowedBy.getID()
							+ " escaped.\n");
		...
		ObstacleReport report = game.getBoard().isObstacleOnPath(position,
				rotation, distance, owner, game.getPlayer(owner).getAllies());

		if (!game.getPlayer(ship.getOwner()).isAlly(report.hexOwner)) {
			// par. 5.5.2
			if (checkIfPlayerControlsLocation(ship, report.hexOwner,
					MarinesCompartment.DECK, true)) {
		...
		if (report.hexTerrainType == Terrain.SHALLOW) {
			// par. 17.1
			if (ship.getLoad(CargoType.SILVER) == 0
					&& rollDice(ship, currentPlayer) > 4
					|| ship.getLoad(CargoType.SILVER) != 0
					&& rollDice(ship, currentPlayer) > 2) {
		...
		if (report.hexShip != null) {
			...
			switch (rotation) {
			case N:
				ship.setPosition(position.getA(), position.getB() + distance
						- 1);
				break;
			case NE:
				ship.setPosition(position.getA() + distance - 1,
						position.getB() + distance - 1);
				break;
			case SE:
				ship.setPosition(position.getA() + distance - 1,
						position.getB());
		...
		if (shipTowedBy != null) {
			...
			if (hexOwner != Player.NONE && hexOwner != shipTowedBy.getOwner()) {
				// par. 5.5.2
				if (checkIfPlayerControlsLocation(shipTowedBy, hexOwner,
						MarinesCompartment.DECK, true))
			...
			if (game.getBoard().getHex(towedNewCrd).terrain == Terrain.SHALLOW) {
				// par. 17.1
				if (shipTowedBy.getLoad(CargoType.SILVER) == 0
						&& rollDice(ship, currentPlayer) > 4
						|| shipTowedBy.getLoad(CargoType.SILVER) != 0
						&& rollDice(ship, currentPlayer) > 2) {
					MainBoard.addMessage("Ship #" + shipTowedBy.getID()
							+ " ran aground.\n");
		...
		if (!tugChangedOwner && !crashed && !towedChangedOwner && !ranAground
				&& !rammed)
			MainBoard.addMessage("Ship #" + ship.getID()
					+ ": succesfully moved.\n");
		else
			MainBoard.addMessage("Ship #" + ship.getID()
					+ ": movement problem occured (new position is ["
					+ (ship.getPosition().getA() + 1) + ","
					+ (ship.getPosition().getB() + 1) + "])\n");
	}

	private static void captureShip(Ship ship, PlayerClass newOwner) {
		MainBoard.addMessage("Ship #" + ship.getID() + ": captured by "
				+ newOwner.toString() + "\n");
		...
		for (Player plr : Player.values()) {
			...
			for (MarinesCompartment location : MarinesCompartment.values()) {
				...
				newOwner.addMarinesInterned(plr,
						ship.getMarinesNumber(plr, location, Commons.BOTH));
				killMarines(ship, plr, location,
						ship.getMarinesNumber(plr, location, Commons.BOTH),
						KillingMode.WITHOUT_COMMANDER);

				if (ship.getCommanderState(plr, location) != CommanderState.NOT_THERE) {
					if (newOwner.isAlly(plr)
							&& ship.getCommanderState(plr, location) == CommanderState.IMPRISONED) {
						ship.setCommander(plr, location, CommanderState.READY);
					}
					if (!newOwner.isAlly(plr)) {
						Coordinate pos = ship.getPosition();
						if (game.getBoard().getHex(pos).owner == newOwner
								.getIdentity()) {
							newOwner.addCommandersInterned(plr);
							ship.setCommander(plr, location,
									CommanderState.NOT_THERE);
						} else {
							ship.setCommander(plr, location,
									CommanderState.NOT_THERE);
	...
	private static void internShip(Ship ship, Player newOwner) {
		MainBoard.addMessage("Ship #" + ship.getID() + ": interneed by "
				+ newOwner.toString() + "\n");
	...
	public static MovementEvent forcedShipMovement(Ship ship,
	 ...
	 */
	{
		...
		if (mode == ShipMovementMode.MOVE_WRECK_NORMAL
		...
		Player owner = ship.getOwner();
		ObstacleReport report = game.getBoard().isObstacleOnPath(position,
				rotation, 1, owner, game.getPlayer(owner).getAllies());

		if (report.hexOwner != Player.NONE) {
			if (ship.getMarinesNumber(report.hexOwner, MarinesCompartment.DECK,
					Commons.BOTH) > 0 || ship.getOwner() == Player.NONE) {
				captureShip(ship, game.getPlayer(report.hexOwner));
		...
		if (report.hexTerrainType == Terrain.SHALLOW) {
			// par. 15.5, 17.1
			if (ship.getLoad(CargoType.SILVER) == 0
					&& rollDice(ship, ship.getOwner()) > 4
					|| ship.getLoad(CargoType.SILVER) != 0
					&& rollDice(ship, ship.getOwner()) > 2
					|| ship.isParameter(Parameter.IS_WRECK)) {
				MainBoard.addMessage("Ship #" + ship.getID()
						+ ": ran aground.\n");
				ship.setParameter(Parameter.IS_IMMOBILIZED);
		...
		if (report.hexShip != null) {
			MainBoard.addMessage("Ship #" + report.hexShip.getID()
					+ ": rammed by ship" + ship.getID() + ".\n");
	...
	public static void rotateShip(Ship ship, int _angle) {
		ship.setRotation(RotateDirection.valueOf((ship.getRotation().ordinal()
				+ _angle + 6) % 6));
	...
	public static int getGroupsNumber(Ship ship, MarinesCompartment location) {
		return DataExtractors.getCompartmentGroups(ship,
				location).split("#").length;
	}

	public static void shoot(Ship ship, Ship targetShip, int distance,
			GunCompartment compartment, Gun ownGunType, AimPart aimedPart,
			Gun aimedGunType) {
		...
		// par. 12.7
		int groupNumber = getGroupsNumber(ship, MarinesCompartment.BATTERIES);
		if (groupNumber > 0)
			accuracyBonus = 1;
		// --
		...
		if (aimedPart == AimPart.CANNON) {
			if (rollDice(ship, currentPlayer) >= 5) {
				MainBoard.addMessage("Ship #" + ship.getID()
						+ ": shot hit the part.\n");
				if (destroyCannonIS(targetShip,
						calculateCompartmentToAim(ship, targetShip),
						aimedGunType, MsgMode.ON)) {
					killMarines(ship, Player.NONE,
							MarinesCompartment.BATTERIES,
							aimedGunType.getCrewSize(),
							KillingMode.WITHOUT_COMMANDER);
					calculateShipOwner(ship);
				}
			} else
		...
		if (aimedPart == AimPart.HULL) {
			...
			switch (roll) {
			case 1:
				marinesToKill = Math.min(1, targetShip.getCannonsNumber(comp,
						Gun.HEAVY, Commons.BOTH))
						* Gun.HEAVY.getCrewSize();
				break;
			case 2:
				marinesToKill = Math.min(2, targetShip.getCannonsNumber(comp,
						Gun.LIGHT, Commons.BOTH))
			...
			killMarines(ship, Player.NONE, MarinesCompartment.BATTERIES,
					marinesToKill, KillingMode.WITHOUT_COMMANDER);
			calculateShipOwner(ship);
		}
	}
	...
	public static int getDistanceToMove(Ship ship) {
		...
		int currentSpeed = Math.min(
				ship.getMast(),
				getAlliedMarinesNumber(ship, currentPlayer,
						MarinesCompartment.DECK) / 2);
		...
		int delta = Math.abs(game.getWindDirection().ordinal()
				- rotation.ordinal());
		if (delta < 3)
		...
		if (towedShip != null) {
			...
			// [16.5].
			if (!checkIfPlayerControlsLocation(towedShip, currentPlayer,
					MarinesCompartment.DECK, true))
				return 0;
		...
		for (maxDistance = 1; maxDistance <= maxDist_; maxDistance++) {
			...
			if (towedShip != null) {
				if (!game.getPlayer(previousHexOwner).isAlly(
						towedShip.getOwner()))
					break;
			...
			Player owner = ship.getOwner();
			report = game.getBoard().isObstacleOnPath(position, rotation,
					maxDistance, owner, game.getPlayer(owner).getAllies());

			// Require confirmation for ramming, shallow water access etc.
			if (report.hexOwner != Player.NONE
					|| report.hexTerrainType == Terrain.SHALLOW
					|| report.hexTerrainType == Terrain.ISLAND
					|| report.hexShip != null)
				break;
		...
		if (ship.isCommanderOnboard(currentPlayer)
				&& game.getPlayerFleetSize(currentPlayer) > 1) {
			if (report.hexTerrainType == Terrain.ISLAND
					|| !Board.isOnMap(newPosition))
				return Math.min(0, maxDistance - 1);

			if (report.hexShip != null
					&& ship.getDurability() <= report.hexShip.getShipClass()
							.getDurabilityMax() / 2)
				return Math.min(0, maxDistance - 1);
		}
	...
	public static boolean checkIfBoardable(Ship source, Player player,
			Ship target) {
		/*
		...
		if (Math.abs(sourcePos.getA() - targetPos.getA()) > 1
				|| Math.abs(sourcePos.getB() - targetPos.getB()) > 1)
			return false; // 12.1
	 ...
	 * @return
	 */
	public static HandlingPartner checkIfHandleable(Ship source, Player player,
			Ship target) {
		// par. 14.1
	...
	public static boolean checkIfBoardingEscapePossible(Ship ship, Player player) {
		...
		// par. 12.8.1
		if (!checkIfPlayerControlsLocation(ship, currentPlayer,
				MarinesCompartment.DECK, false))
			return false;
		// --

		// par. 12.8.2
		if (Math.abs(game.getWindDirection().ordinal()
				- ship.getRotation().ordinal()) == 3)
			return false;

		if (ship.isParameter(Parameter.IS_IMMOBILIZED)
				|| ship.isParameter(Parameter.IS_WRECK) || ship.getMast() == 0
				|| ship.getTowedBy() != null)
			return false;

		Ship towed = ship.getTowOther();
		if (towed != null) {
			if (game.getWindDirection().ordinal()
					- ship.getRotation().ordinal() == 0)
				return false; // par. 16.12

			if (towed.isParameter(Parameter.IS_IMMOBILIZED))
				return false;

			if (!checkIfPlayerControlsLocation(towed, currentPlayer,
					MarinesCompartment.DECK, true))
		...
		if (!Board.isOnMap(position.sum(shift))) {
			if (game.getBoard().getHex(position).owner != currentPlayer
					|| ship.isCommanderOnboard(currentPlayer)
	...
	public static void deployCommander(Ship ship) {
		...
		for (Ship s : game.getPlayer(currentPlayer).getFleet()) {
			if (s.getCommanderState(currentPlayer, MarinesCompartment.DECK) == CommanderState.READY) {
				s.setCommander(currentPlayer, MarinesCompartment.DECK,
						CommanderState.NOT_THERE);
				break;
			}
		}

		ship.setCommander(currentPlayer, MarinesCompartment.DECK,
				CommanderState.READY);
	}

	public static GunCompartment calculateCompartmentToAim(Ship source,
			Ship target) {
		// XXX: okret target strzela do source (?)
		...
		return game.getBoard().calculateCompartmentToAim(crd.getA(),
				crd.getB(), target.getRotation().ordinal());
	}
