...
public class MarinesSection {
	...
	private MarinesSection() {
		...
		for (MarinesCompartment mc : MarinesCompartment.getShipCompartments()) {
			marines.put(mc,
					new ReusableMap<Player>(Player.class, Player.getValues()));
	...
	public MarinesSection(MarinesSection ms) {
		...
		for (MarinesCompartment mc : MarinesCompartment.getShipCompartments()) {
			for (Player p : Player.getValues()) {
				marines.get(mc).getElement(p)
						.copy(ms.marines.get(mc).getElement(p));
	...
	public MarinesSection(ShipClass sc, Player shipOwner) {
		...
		marines.get(MarinesCompartment.DECK).getElement(shipOwner)
				.setReady(sc.getCrewDeckMax());
		marines.get(MarinesCompartment.BATTERIES).getElement(shipOwner)
				.setReady(sc.getCrewMax() - sc.getCrewDeckMax());
	}

	public int getMarinesNumber(Player player, MarinesCompartment location,
			int state) {
		switch (state) {
		case Commons.READY:
	...
	public CommanderState getCommanderState(Player player,
			MarinesCompartment location) {
		return commanders.get(location).get(player);
	}

	public void moveMarines(Player player, MarinesCompartment source,
			MarinesCompartment destination, int amount) {
	...
	public void setCommander(Player player, MarinesCompartment location,
			CommanderState state) {
		commanders.get(location).put(player, state);
	}

	public void moveCommander(Player player, MarinesCompartment source,
			MarinesCompartment destination) {
		if (source != MarinesCompartment.SHIP_X) {
			if (commanders.get(source).get(player) != CommanderState.READY)
				throw new IllegalArgumentException("commander not ready");
	 ...
	 *         dowódcą/bez dowódcy
	 */
	public Object[] killMarines(Player player, MarinesCompartment location,
			int amount, KillingMode mode) {
		Object[] ret = new Object[2];
		ret[1] = KillingMode.WITHOUT_COMMANDER;

		int currentAmount = amount;

		if (currentAmount <= marines.get(location).getElement(player)
				.getReady()) {
			marines.get(location).getElement(player)
					.modifyReady(-currentAmount);
			ret[0] = amount;
			return ret;
		}
	...
	public void useMarines(Player player, MarinesCompartment compartment,
			int number) {
